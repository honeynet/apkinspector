    ...
    def show(self, nodeList, linkList):    
        ...
        for node in nodeList:
            ...
            for i in range (1, lineNumber+1):
                ...
                nodeNew.setParentItem(self.root)
 ...
 #           node.setParentItem(self.root)
    ...
    def showcall(self, nodeList, linkList):    
        ...
        for node in nodeList:
            ...
            nodeNew = Node (0,0,0,0)
            nodeNew.left = node.left
            nodeNew.top = node.top
            nodeNew.width = node.width
            nodeNew.height = node.height
            nodeNew.myText = node.myText

            print "Node created with (%s %s %s %s)" % (nodeNew.left, nodeNew.top, nodeNew.width, nodeNew.height)
...
class Link(QGraphicsLineItem):
    ...
    def setColor(self, color):
        colorDir = {"green":Qt.green, "red":Qt.red, "blue":Qt.blue}
...
#Yuan Full display of hint
    def setHint(self,text):
        self.fullText = str(text)
        self.update()
...
# Yuan modify display
        ...
        rect = metrics.boundingRect(self.sText)
...
#Yuan modify display
        self.sText = self.myText.replace("Ljava/lang","")
        self.sText = self.myText.replace("Landroid/content"," ")
        self.sText = self.myText.replace("const","st")
        self.sText = self.myText.replace("Long","l")
        self.sText = self.myText.replace("invoke","i")
  
       
        painter.drawText(rect, Qt.AlignLeft | Qt.AlignVCenter, self.sText)
...
#            self.setText(text)
        ...
        inputDialog_dalvik = inputDialog(self, self.fullText)
